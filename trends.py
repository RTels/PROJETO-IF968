...
def has_sentiment(s):
    ...
    if type(s['value']) == float or type(s['value']) == int:
        return True
    else:
        return False 
...
def analyze_tweet_sentiment(tweet):
    ...
    for i in lista_palavras:
        if has_sentiment(get_word_sentiment(i)) == True:
               tot_palavras += 1
               somatorio += sentiment_value(get_word_sentiment(i))
    if tot_palavras > 0:
        media = (somatorio/tot_palavras)
...
def area(t):
    ...
    n = len(t)
    for i in range(n-1):
        soma += (t[c][0] * t[c+1][1]) - (t[c+1][0] * t[c][1])
        c += 1
    area = (soma/2)
    return (area)
...
def cx(t):
    ...
    for i in range(len(t)-1):
        soma += (t[c][0] + t[c+1][0]) * ((t[c][0]*t[c+1][1]) - t[c+1][0]*t[c][1])
        c += 1
    cx = (soma/(6*area(t)))
    return cx
...
def cy(t):
    ...
    c = 0
    for i in range(len(t)-1):
        soma += (t[c][1]+t[c+1][1]) * ((t[c][0]*t[c+1][1]) - t[c+1][0]*t[c][1])
        c += 1
    cy = (soma/(6*area(t)))
    return cy
...
def find_centroid(polygon):
    ...
    """
    if area(polygon) == 0:
        return (polygon[0][0], polygon[0][1], int(area(polygon)))
    return (float(cx(polygon)),float(cy(polygon)),(abs(area(polygon))))
...
def find_center(polygons):
    ...
    a_total = 0
    for i in polygons:
        tupla = find_centroid(i)
        pontox += tupla[0]*tupla[2]
        pontoy += tupla[1]*tupla[2]
        a_total += tupla[2]
    pontox = pontox/ a_total
    pontoy = pontoy/ a_total
...
def find_closest_state(tweet, state_centers):
    ...
    lista = []
    for i in state_centers.items():
        lista.append((geo_distance(local,i[1]),i[0]))
    min_dist = min(lista)
...
def group_tweets_by_state(tweets):
    ...
    """
    tweets_by_state = {}
    us_centers = {i: find_center(x) for (i, x) in us_states.items()}
    for i in us_centers:
        tweets_by_state[i] = []
    for i in tweets:
        estado = find_closest_state(i, us_centers)
        tweets_by_state[estado].append(i)
...
def most_talkative_state(term):
    ...
    dici = group_tweets_by_state(tweets)
    my_Max = 0
    for i in dici:
        if len(dici[i]) > my_Max:
            my_Max = len(dici[i])
            result = i
    return result
